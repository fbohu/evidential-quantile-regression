...
def NIG_NLL(y, gamma, v, alpha, beta, w_i_dis, quantile, reduce=True):
    ...
    #twoBlambda = 2.0*beta*(1.0+tau_two*tf.expand_dims(w_i_dis.mean(),1)*v)
    ...
    #return tf.reduce_sum(nll) if reduce else nll
...
def NIG_Reg(y, gamma, v, alpha, beta, w_i_dis, quantile, omega=0.01, reduce=True, kl=False):
    ...
    if kl:
        ...
        # this is one is good as fuck
        #evi = 2.0*v+alpha+1/beta#+w*2/beta#+1/beta
        reg = error*evi

    #return tf.reduce_sum(reg) if reduce else reg
    return tf.reduce_mean(reg) if reduce else reg

def EvidentialRegression(y_true, gamma, v, alpha, beta, quantile, coeff=1.0, reduce=True):
    tfd = tfp.distributions
    theta = (1.0-2.0*quantile)/(quantile*(1.0-quantile))
    
    #mean_ = tf.math.sqrt(beta/(alpha-1))
    mean_ = beta/(alpha-1)
